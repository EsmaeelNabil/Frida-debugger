  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("GET_DEVICES", async () => {
      try {
        ...
        socket.emit(
          "ERROR",
          ErrorHandler.handle(error as Error, "GET_DEVICES"),
        );
      }
    });
    ...
    socket.on(
      "GET_APP",
      async (data: { deviceId: string; appName: string }) => {
        try {
          console.log("GET_APP", data);
          const app = await deviceManager.getFullApp(
            data.deviceId,
            data.appName,
          );
          socket.emit("APP", app);
    ...
    socket.on("GET_ALL_APPS", async (deviceId: string) => {
      try {
        ...
        socket.emit(
          "ERROR",
          ErrorHandler.handle(error as Error, "GET_ALL_APPS"),
        );
      }
    });
    ...
    socket.on("GET_PROCESSES", async (deviceId: string) => {
      try {
        ...
        socket.emit(
          "ERROR",
          ErrorHandler.handle(error as Error, "GET_PROCESSES"),
        );
      }
    });
    ...
    socket.on("UNLOAD_SCRIPTS", async () => {
      try {
        ...
        socket.emit(
          "ERROR",
          ErrorHandler.handle(error as Error, "UNLOAD_SCRIPTS"),
        );
      }
    });
    ...
    socket.on(
      "ATTACH_TO_APP",
      async (data: { deviceId: string; appName: string; script: string }) => {
        try {
          ...
          socket.emit(
            "ERROR",
            ErrorHandler.handle(error as Error, "ATTACH_TO_APP"),
          );
        }
      },
    );
    ...
    socket.on(
      ...
      async (data: {
        ...
        try {
          ...
          socket.emit(
            "ERROR",
            ErrorHandler.handle(error as Error, "LAUNCH_APP"),
          );
        }
      },
    );
